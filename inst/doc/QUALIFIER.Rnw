...
Here "outlier.cutoff" is used and threshold "lBound" is specified ("less than", use uBound for "larger than"). 
...
has significant variance and is marked as the group outlier. The formula supplied here in the plot method overwrites the one stored 
in the qaTask object, thus change the way of viewing the data.
