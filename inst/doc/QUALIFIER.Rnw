...
\Robject{stats} in the data environment is the data frame that stores the statistics.
 
<<viewStats, eval=FALSE ,echo=true, results=verbatim>>=
...
has significant variance and is marked as the group outlier. The formula supplied here in the plot method can overwrite the one stored 
...
All the lattice options (like \Robject{scales} here in this example) can be passed to control the appearance of trellis plot.
...
The complete example of the report can be viewed at:
http://mikejiang.github.com/QUALIFIER/qaReport/new/index.html 
