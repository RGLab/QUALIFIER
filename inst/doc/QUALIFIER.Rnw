...
\noindent \textbf{Background}
  The current \Rpackage{flowQ} package does the quality assessment on the ungated FCM data. However, there is a need to
  ...
  populations (such as white blood cells, lymphacytes, monocytes etc). The \Rpackage{flowQ} package was also not designed for dealing with
...
Since we are only interested in the gating strategy (gates defined in the template),\Robject{execute} can be set as FALSE  
to skip the actual gating process. \Robject{useInternal} is set as TRUE to use internal structure (c++) for faster parsing. 
Please see the documentation of \Rpackage{flowWorkspace} package for more details regarding how to use \Rfunction{parseWorkspace} function. 
...
\section{Apply the gating template to the data}
Then gating hierarchy \Robject{gh\_template} containing the actual template gates is extracted
from the result \Robject{GatingSet} "\Robject{GT}". 
<<gh_template, eval=FALSE, echo=TRUE,results=verbatim>>=
gh_template<-GT[[1]]					
@

The \Rfunction{GatingSet} method here is the contructor that takes a \Robject{gatingHierarchy} object as the template and a list of FCS file names (\Robject{filenames}) that need the quality assurance.
The result "G" is the \Robject{GatingSet} that contains the gated data and some of the cell population statistics that can be viewed by \Robject{getPopStats} method.
<<gh_template, eval=FALSE, echo=TRUE,results=verbatim>>=
##datapath is the path where FCS files stores
G<-GatingSet(gh_template,filenames,path="./data")
getPopStats(G[[1]])
@
Optionally, \Robject{isNcdf} can be set to TRUE to support netCDF storage of large flow datasets when memory resource is limited. 
...
\Robject{db} is an environment that serves as a container storing all the QA data and results.
Firsly, \Rfunction{initDB} function initializes and creates the data structures for QA. 
Then \Rfunction{qaPreprocess} is a convenient wrapper that calls underlining routines (\Rfunction{getQAStats},\Rfunction{saveToDB})
...
\Robject{metaFile} is a csv file that contains the extra annoations about each FCS file.It should at least have one column
...
@
\Robject{fcs.colname} and \Robject{date.colname} are the arguments to identify the columns in the meta data (the flat table stored as a csv file)
...
\Rfunction{read.qaTask} reads external csv spreadsheet that contains the descriptions of each QA task and creates a list of \Rclass{qaTask} objects. 

<<read.qaTask, echo=true, results=verbatim>>=
checkListFile<-file.path(system.file("data",package="QUALIFIER"),"qaCheckList.csv.gz")
qaTask.list<-read.qaTask(db,checkListFile=checkListFile)
qaTask.list[1:2]
@
The \Rclass{qaTask} can also be created individually by the contructor \Rfunction{makeQaTask}.

\section{Quality assessment and visualiztion} 
\Rfunction{qaCheck} and \Rfunction{plot} are the two main methods to perform the quality assessment and visualize the QA results.
...
It is generally of the form y $\sim$ x | g1 * g2 * ... , where y is the statistics to be checked in this QA and must be one of the four types:
...
x specifies the variable plotted on x-axis (such as date) in the \Rfunction{plot} method.
...
\Rfunction{qaCheck} reads all the necessary information about the gated data from \Rclass{qaTask} object.
...
Here \Rfunction{outlier.cutoff} provided by the package is used and threshold "lBound" is specified ("less than", use uBound for "larger than"). 

After the outliers are called, the results can be plotted by \Rfunction{plot} method.
...
\Rfunction{clearCheck} is the method to removes the outlier results detected by the previous \Rfunction{qaCheck} call for the specific qaTask.
<<clearCheck,results=verbatim>>=
...
has significant variance and is marked as the group outlier. By default \Rfunction{qaCheck} uses normal-distribution-based outlier function to detect group outliers.  
User-defined function can also supplied through \Robject{gOutlierfunc} argument.Again the function should take a numeric vector as input and returns a logical vector as the output.
...
With \Robject{scatterPlot} and \Robject{subset} arguments, scatter plots can be generated for the selected FCS files or sample groups,
...
All the lattice options (like \Robject{scales} here in this example) can be passed to control the appearance of trellis plot,see the documentation of \Rpackage{lattice} for more details.
...
@
\Robject{tubesEvents} pass to the argument of \Rfunction{qaCheck} could be a one-column data frame or a named list/vector. 
...
qaCheck(qaTask.list[["RedundantStain"]],gOutlierfunc = list(func = outlier.norm, args =list(z.cutoff = 1)))
...
\Robject{plotAll} is the argument to control the plotting of the individual scatter plot for each FCS file. If TRUE, scatter plots for all the FCS files are generated. 
...
\Robject{subset} is the filter to only generate the report on a subset of flow data.
...
If there is no outlier is detected, the qaTask is not plotted by default. In order to change this setting, \Rfunction{htmlReport} can be set as TRUE to plot the qaTask regardlessly.      
This is particually useful for the task that tracks the longitudinal trend instead of the individual outliers.
Besides the lattice arguments that can be configured by \Rfunction{qpar} (for the summary xyplot and bwplot) and \Rfunction{scatterPar} (for the individual FCS xyplot).
\Robject{xlog},\Robject{ylog} flag can be set for scatter plot to transform the flow data to log scale for the proper display of the stained channels.
<<qa.report,eval=FALSE, echo=true, results=verbatim>>=
...
Here is the \href{http://mikejiang.github.com/QUALIFIER/qaReport/new/index.html}{example} of the QA report.
...
And \Rfunction{plot} only reads the outlier detection results pre-calculated by \Rfunction{qaCheck},
which reduces the cost of interactive visualization.

Two kinds of lattice plots are currently supported:xyplot and bwplot(boxplot), depending on the \Robject{plotType} in \Rfunction{qaTask} object.
