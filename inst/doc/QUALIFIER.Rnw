...
to skip the actual gating process. \Robject{useInternal} is set as TRUE to use internal structure(c++) for the faster parsing. 
...
The \Rfunction{GatingSet} method here is the contructor that takes a \Robject{gatingHierarchy} object as the template
and and a list of FCS file names (\Robject{filenames}) that need the quality assurance.
...
getPopStats(G[[1]])
@
Optionally,\Robject{isNcdf} can be set as TRUE to support netCDF storage of large flow datasets when memory resource is limited. 
...
to calculates/extracts statistics of gated cell populations and save them along with the gating set and the extra FCS meta data.
...
@
\Robject{fcs.colname} and \Robject{date.colname} are the arguments to identify the columns in the meta data (the flat table stored as a csv)
that specifies the FCS filenames and the dates (to be formatted as to "\%m/\%d/\%y"). 
\Robject{stats} in the data environment is the data frame that stores the statitics.
...
qaTask.list[1:2]
@
The \Rclass{qaTask} can also created individually by the contructor \Rfunction{makeQaTask}.
...
All the lattice options (like \Robject{scales} here in this example) can be passed to control the apparance of trellis plot.
...
This is particually useful for the task that trackes the longitudinal trend instead of the individual outliers.
...
xlog(ylog) flag can be set for scatter plot to transform the flow data to log scale for the proper displaying those stained channels.
<<qa.report,eval=FALSE, echo=true, results=verbatim>>=
...
The complete example of the report can be viewed at http://mikejiang.github.com/QUALIFIER/qaReport/new/index.html. 
