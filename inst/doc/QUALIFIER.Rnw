...
<<changePath,echo=FALSE,result=hide,eval=TRUE>>=
ncFlowSet(db$G)@file<-file.path("/home/wjiang2/rglab/workspace/QUALIFIER/output/",ncFlowSet(db$G)@file)
...
<<plot-RBCLysis-subset2, fig=true, results=verbatim>>=
plot(qaTask.list[["RBCLysis"]],subset="name=='06087181_F01_I010.fcs'",scatterPlot=T)
@
...
<<plot-MNC-scatter, fig=true, results=verbatim>>=
