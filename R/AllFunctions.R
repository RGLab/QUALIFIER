...
qaPreprocess_labkey<-function(db=.db,gs,gsid,metaFile,fcs.colname="name",date.colname=NULL,...)
{
	##associate the anno with gating set and save them in db
	saveToDB_labkey(db,gs,gsid,metaFile,fcs.colname,date.colname)
...
saveToDB_labkey<-function(db=.db,gs,gsid,metaFile,fcs.colname="name",date.colname=NULL)
{
    idColName <-qa.par.get("idCol")
